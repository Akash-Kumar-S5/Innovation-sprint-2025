...
4. Download and replace the credentials.json file
    ...
    async performOAuthFlow(tokenPath) {
...
NOTE: You can ignore any "localhost refused to connect" errors - just copy the code!
    ...
    async exchangeCodeForTokens(code, tokenPath) {
        try {
            ...
            try {
                ...
                switch (name) {
                    case "read_google_doc":
                        ...
                        return await this.readGoogleDoc(args.docId);
                    case "search_google_docs":
                        if (!args?.query) {
                            throw new Error("Search query is required");
                        }
                        return await this.searchGoogleDocs(args.query);
    ...
    async readGoogleDoc(docId) {
        try {
            ...
            if (typeof error === 'object' && error !== null && 'code' in error && error.code === 404) {
                ...
                errorMessage = `Error reading document: ${error.message}`;
            }
            else {
    ...
    async searchGoogleDocs(query) {
        try {
            ...
            const resultText = results.map((doc, index) => `${index + 1}. ${doc.name}\n   ID: ${doc.id}\n   Modified: ${doc.modified}\n   Link: ${doc.link}\n`).join('\n');
            return {
                content: [{
                        type: "text",
    ...
    async listGoogleDocs() {
        try {
            ...
            const docList = docs.map((doc, index) => `${index + 1}. ${doc.name}\n   ID: ${doc.id}\n   Modified: ${doc.modified}\n   Link: ${doc.link}\n`).join('\n');
            return {
                content: [{
                        type: "text",
